...
collects the whole folder, prints a line per file and ends with the count. There are sixty-five
...
- **A test you haven't seen fail doesn't count.** Break the code on purpose, watch it go red,
  ...
  `tests/test_mutantes.py` breaks one hundred and seventeen real guards, one at a time, on a copy of the
  tree, and
