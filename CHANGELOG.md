...
**Reabrir con otro modelo bifurca la sesión, y el `[m]` aparece también con un solo lanzador.** Dos flecos de la feature de arriba. (1) `claude --resume <id> --model X` dejaba el transcript con dos modelos dentro, y de ahí es de donde sereno deduce el modelo y el tope de contexto de la fila: ahora, si se elige modelo al reabrir, va además `--fork-session` (verificado en `claude --help`): la sesión vieja queda intacta y la nueva arranca con id propio; con `por defecto` no hay fork. (2) El cuadro de `r` solo se abría con dos o más lanzadores, y con uno solo no había dónde elegir modelo: ahora se abre también con uno cuando hay modelo que ofrecer (`_modelos_para`, compartido con el cuadro). De paso, el `ejecutar` de `--demo` no aceptaba el modelo y `[m]` reventaba ahí. Casos en `test_cuadros_de_eleccion.py` y `test_donde_abrir.py`, dos mutantes (162 → 164).
...
**La tabla comparativa del README dejó de mentir.** La rejilla de ✅/❌ daba por imposibles en los rivales cosas que sí hacen: el picker nativo de `claude --resume` lista sesiones cross-project y muestra la rama git; claude-command-center anuncia el contexto restante por fila y cubre varios motores. Un lector que conociera cualquiera de los dos descontaba el resto del README. Ahora la tabla dice la verdad —cada capacidad existe en algún otro sitio— y defiende lo que de verdad es único: tenerlas todas, en un fichero de stdlib, por SSH, sin dependencias.
